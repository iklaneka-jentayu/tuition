...
function myInitCode(){
    ...
    setTimeout(() => {
        ...
        addNewBtn.addEventListener('click', function() {
            updateModalTitle('admin.modal.addStudent');
        ...
        modalSave.addEventListener('click', function() {
            ...
            if (!validateStudentForm(studentData)) {
                showNotification(getTranslation('admin.validation.required'), 'error');
                return;
            }
        ...
        document.addEventListener('click', function(e) {
            ...
            if (e.target.closest('.btn-delete')) {
                ...
                document.getElementById('confirmMessage').textContent = 
                    `${getTranslation('admin.confirm.deleteItem')} "${studentName}"?`;
        ...
        document.getElementById('refreshBtn').addEventListener('click', function() {
            loadStudents();
            showNotification(getTranslation('admin.refresh.success'), 'success');
        });
        ...
        document.getElementById('processPaymentBtn').addEventListener('click', function() {
            showNotification(getTranslation('admin.payments.processNote'), 'info');
        });
        
        // Quick Actions
        document.getElementById('sendBulkEmail').addEventListener('click', function() {
            showNotification(getTranslation('admin.actions.emailNote'), 'info');
        });
        ...
        document.getElementById('importData').addEventListener('click', function() {
            showNotification(getTranslation('admin.actions.importNote'), 'info');
        });
        ...
        function loadStudents(page = 1) {
            ...
            pageData.forEach(student => {
                ...
                row.innerHTML = `
                    ...
                    <td>${student.plan}</td>
                    <td><span class="status ${student.status}">${getStatusTranslation(student.status)}</span></td>
                    <td>
                        <div class="action-buttons">
                            <button class="btn-edit" title="${getTranslation('admin.actions.edit')}">
                                <i class="fas fa-edit"></i>
                            </button>
                            <button class="btn-delete" title="${getTranslation('admin.actions.delete')}">
                                <i class="fas fa-trash"></i>
                            </button>
                            <button class="btn-view" title="${getTranslation('admin.actions.view')}">
        ...
        function loadEnrollments() {
            ...
            tableBody.innerHTML = enrollments.map(enrollment => `
                <tr>
                    ...
                    <td><span class="status ${enrollment.paymentStatus}">${getStatusTranslation(enrollment.paymentStatus)}</span></td>
                    <td><span class="status ${enrollment.status}">${getStatusTranslation(enrollment.status)}</span></td>
                    <td>
                        <button class="btn-view" title="${getTranslation('admin.actions.view')}">
                            <i class="fas fa-eye"></i>
                        </button>
        ...
        function loadPayments() {
            ...
            tableBody.innerHTML = payments.map(payment => `
                <tr>
                    ...
                    <td><span class="status ${payment.status}">${getStatusTranslation(payment.status)}</span></td>
                    <td>
                        <button class="btn-view" title="${getTranslation('admin.actions.receipt')}">
        ...
        function addStudent(studentData) {
            ...
            showNotification(getTranslation('admin.student.added'), 'success');
        }
        ...
        function editStudent(studentId) {
            ...
            if (student) {
                ...
                updateModalTitle('admin.modal.editStudent');
                studentModal.classList.add('active');
            }
        }
        ...
        function updateStudent(studentId, updatedData) {
            ...
            if (index !== -1) {
                ...
                showNotification(getTranslation('admin.student.updated'), 'success');
            }
        }
        ...
        function deleteStudent(studentId) {
            ...
            showNotification(getTranslation('admin.student.deleted'), 'success');
        }
        ...
        function exportToCSV() {
            ...
            if (students.length === 0) {
                showNotification(getTranslation('admin.export.empty'), 'error');
                return;
            }
            ...
            showNotification(getTranslation('admin.export.success'), 'success');
        }
        
        function generateReport() {
            showNotification(getTranslation('admin.report.generating'), 'info');
            ...
            setTimeout(() => {
                ...
                showNotification(getTranslation('admin.report.generated'), 'success');
        ...
        function backupData() {
            ...
            showNotification(getTranslation('admin.backup.success'), 'success');
        }
        ...
        function getTranslation(key) {
            const translations = {
                en: {
                    'admin.validation.required': 'Please fill in all required fields',
                    'admin.student.added': 'Student added successfully',
                    'admin.student.updated': 'Student updated successfully',
                    'admin.student.deleted': 'Student deleted successfully',
                    'admin.refresh.success': 'Data refreshed successfully',
                    'admin.export.empty': 'No data to export',
                    'admin.export.success': 'Data exported successfully',
                    'admin.report.generating': 'Generating report...',
                    'admin.report.generated': 'Report generated successfully',
                    'admin.backup.success': 'Backup created successfully',
                    'admin.payments.processNote': 'Payment processing would be implemented here',
                    'admin.actions.emailNote': 'Bulk email feature would be implemented here',
                    'admin.actions.importNote': 'Data import feature would be implemented here',
                    'admin.confirm.deleteItem': 'Are you sure you want to delete',
                    ...
                    'admin.actions.receipt': 'View Receipt'
                },
                ms: {
                    'admin.validation.required': 'Sila isi semua ruangan yang diperlukan',
                    'admin.student.added': 'Pelajar berjaya ditambah',
                    'admin.student.updated': 'Pelajar berjaya dikemas kini',
                    'admin.student.deleted': 'Pelajar berjaya dipadam',
                    'admin.refresh.success': 'Data berjaya disegarkan',
                    'admin.export.empty': 'Tiada data untuk dieksport',
                    'admin.export.success': 'Data berjaya dieksport',
                    'admin.report.generating': 'Menjana laporan...',
                    'admin.report.generated': 'Laporan berjaya dijana',
                    'admin.backup.success': 'Sandaran berjaya dibuat',
                    'admin.payments.processNote': 'Pemprosesan pembayaran akan dilaksanakan di sini',
                    'admin.actions.emailNote': 'Ciri emel pukal akan dilaksanakan di sini',
                    'admin.actions.importNote': 'Ciri import data akan dilaksanakan di sini',
                    'admin.confirm.deleteItem': 'Adakah anda pasti mahu memadam',
                    'admin.status.active': 'Aktif',
                    'admin.status.pending': 'Menunggu',
                    'admin.status.inactive': 'Tidak Aktif',
                    'admin.status.suspended': 'Digantung',
                    'admin.actions.edit': 'Sunting',
                    'admin.actions.delete': 'Padam',
                    'admin.actions.view': 'Lihat',
                    'admin.actions.receipt': 'Lihat Resit'
                }
            };
        ...
        function getStatusTranslation(status) {
            return getTranslation(`admin.status.${status}`);
        }
        
        function updateModalTitle(key) {
            const titleElement = document.getElementById('modalTitle');
            if (titleElement.hasAttribute('data-lang')) {
                titleElement.setAttribute('data-lang', key);
            } else {
